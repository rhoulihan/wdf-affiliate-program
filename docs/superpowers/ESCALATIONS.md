...
| 32 | **Affiliate dashboard does not match the Phase-1 data model.** Three of four tabs are non-functional against the 4-state order machine, and `PUT /affiliates/:id` lets an **affiliate set their own `deliveryFee` — which is their commission**. Analysis was done 2026-09-25 and awaits discussion; it is referenced by a memory link that points at no file. | Rick | 2026-09-25 | Agree the scope: which tabs survive, and lock `deliveryFee` to admin-only. The self-set commission is the part that should not wait | — |
